...
function transformAst(entryDir, modulesMap) {
  for (const { ast, path: currentPath, type } of modulesMap.values()) {
    const exporters = {};
    ...
    if (type === "esm") {
      ast.program.body.unshift(
        ...
        t.callExpression(t.identifier("require.__addEsmFlag"), [
    ...
    traverse(ast, {
      ...
      ReferencedIdentifier(path) {
        ...
        const { id: parentModuleId } = getModule(
          modulesMap,
          currentPath,
          parentSrcName,
          entryDir
          ...
          switch (type) {
              ...
              if (path.container.property) {
                if (path.container.property.name === "default") {
                  ...
                  path.parentPath.replaceWith(
                    t.identifier(`${prefix}${parentModuleId}_DEFAULT.d`)
                  );
                } else {
            ...
            case "ImportDefaultSpecifier": {
              if (local.name === localName) {
                assignment = t.identifier(
                  `${prefix}${parentModuleId}_DEFAULT.d`
                );
              }
              break;
            }
            ...
            case "ImportSpecifier": {
              ...
              if (local.name === localName) {
                 ...
                 */
                assignment = t.identifier(
                  `${prefix}${parentModuleId}${`[${JSON.stringify(
                    imported.name
                  )}]`}`
                );
